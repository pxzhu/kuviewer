...
- live Kubernetes mode에서는 선택 Pod의 최근 200줄 logs를 버튼 클릭 시 조회한다. container/initContainer 선택을 지원하고, logs는 저장하지 않으며, RBAC/클러스터 차이로 조회가 안 되면 전체 상세 실패 대신 빈 Logs와 안전한 warning을 표시한다.
...
3. 업로드 모드 강화
   ...
   - 다음 확장 후보는 previous/streaming logs다.
