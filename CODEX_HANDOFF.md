...
- 설치형 클러스터 탐색 앱 packaging spike는 `desktop/packaging-spec.json`, `desktop/README.md`, `scripts/check-desktop-packaging-spec.mjs`로 시작했다. 권장 방향은 Tauri-first, fallback은 Electron이며, 첫 대상은 macOS `.dmg`와 Windows `.exe`다. CI는 desktop packaging spec check를 실행한다.
...
3. 업로드 모드 강화
   ...
   - 다음 확장 후보는 Tauri desktop shell scaffold 또는 saved view metadata polish다.
