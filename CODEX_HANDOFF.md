...
3. Desktop CM session group/card/bulk toolbar와 saved-layout import conflict panel은 표시 모듈로 분리됐고 비핵심 reorder history UI는 제거됐다. 남은 saved-layout folder/preset list JSX를 추가 분리하거나 prototype archive 범위를 결정
