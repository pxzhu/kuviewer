...
3. 업로드 모드 강화
   ...
   - Desktop CM session layout import/export polish는 완료됐다. 설치형/Tauri CM/SSH session panel은 별도 `kuviewer.desktop.cmSessionLayouts` JSON bundle로 saved layout preset만 export/import하며 same-name update, invalid skip, unknown session id prune을 수행하고 CM session export/import schema와 Tauri command payload는 변경하지 않는다.
   - 다음 확장 후보는 Desktop CM session layout conflict preview다.
