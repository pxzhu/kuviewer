...
- 서버/클러스터 연결 시 리소스, 관계, Events, Logs를 읽기 전용으로 탐색할 수 있는 설치형 클러스터 탐색 앱을 장기 목표에 둔다. macOS `.dmg`와 Windows `.exe` packaging 경로는 Tauri-first로 유지한다.
...
- 설치형 클러스터 탐색 앱 packaging spike와 Tauri desktop shell scaffold는 `desktop/packaging-spec.json`, `desktop/README.md`, `desktop/package.json`, `desktop/src-tauri/*`, `scripts/check-desktop-packaging-spec.mjs`로 시작했다. 권장 방향은 Tauri-first, fallback은 Electron이며, 첫 대상은 macOS `.dmg`와 Windows `.exe`다. `desktop-readonly` capability는 shell/fs 권한 없이 `core:default`만 허용한다. Desktop build prerequisites, generated `.icns`/`.ico` icon assets, icon source policy, signing boundaries는 `desktop/BUILD_PREREQUISITES.md`와 machine-checked packaging spec에 기록했다. `.github/workflows/desktop-package.yml`은 manual-only package workflow이며 unsigned build가 기본이다. 2026-06-19에 run `27800527207`로 unsigned macOS DMG dry-run이 성공했고 `Kuviewer_0.1.0_aarch64.dmg` artifact가 업로드됐다. 같은 날 run `27803179419`로 unsigned Windows NSIS EXE dry-run이 성공했고 `Kuviewer_0.1.0_x64-setup.exe` artifact가 업로드됐다. Desktop runtime은 remote Kuviewer server URL profile을 URL-only localStorage metadata로 저장할 수 있고, profile 변경 시 admin token을 지운다. Desktop package workflow는 `package_version` 입력, `KUVIEWER_DESKTOP_VERSION`, `v*` tag ref, fallback `0.1.0` 순서로 package version을 resolve하고 CI workspace의 `desktop/package.json`, `desktop/src-tauri/tauri.conf.json`, `desktop/src-tauri/Cargo.toml`만 빌드 전에 맞춘다. 업로드 artifact 이름은 `kuviewer-macos-dmg-{version}` / `kuviewer-windows-exe-{version}` 형태를 쓴다. `signed` 입력을 켜면 macOS는 CI secrets의 Developer ID `.p12`를 임시 runner keychain에 import하고 Apple notarization env를 Tauri build에 넘긴 뒤 keychain을 삭제한다. Windows는 CI secrets의 PFX를 CurrentUser certificate store에 import하고 thumbprint를 CI workspace Tauri config에 주입한 뒤 certificate를 삭제한다. Local sidecar runtime scaffold는 `scripts/build-desktop-sidecar.mjs`, `desktop/src-tauri/src/main.rs`, `desktop/src-tauri/tauri.conf.json`, `desktop/packaging-spec.json`에 기록됐다. Go API server는 `desktop/src-tauri/binaries/kuviewer-sidecar-{target}`로 빌드되고 generated binaries는 git ignore된다. Tauri `externalBin`은 `binaries/kuviewer-sidecar`로 켜졌고 Rust runtime이 `127.0.0.1:18086` loopback sidecar를 실행한다. Per-launch admin token은 메모리에서 생성되고 `desktop_sidecar_profile` Tauri command로 UI에 전달되어 `sessionStorage`에만 저장된다. Frontend JavaScript에는 shell/fs 권한을 열지 않는다. 기본 sidecar source는 installer smoke용 `mock`이며 `KUVIEWER_DESKTOP_SIDECAR_SOURCE=kubernetes`로 외부 안전 credential source가 있을 때만 Kubernetes provider를 선택한다. `KUVIEWER_DESKTOP_DISABLE_SIDECAR=1`은 remote-only 테스트용이다. Desktop profile panel은 detected local sidecar URL/source를 safe descriptor로만 표시하고, remote profile 사용 중에도 `로컬 sidecar 사용` 버튼으로 sidecar profile을 명시 적용할 수 있다. 버튼 클릭 시 token은 Tauri command에서 다시 읽어 `sessionStorage`에만 저장하고 live mode로 전환한다. Keychain-backed direct-cluster credential design은 `desktop/KEYCHAIN_CREDENTIAL_DESIGN.md`에 기록했다. 첫 runtime 범위는 bearer-token profile이며 secret material은 macOS Keychain 또는 Windows Credential Manager에만 저장하고, sidecar에는 `KUVIEWER_KUBE_TOKEN_FILE` runtime temp file로 전달한다. 현재 runtime은 `desktop_kubernetes_profiles` / `desktop_select_kubernetes_profile` Tauri command로 safe metadata만 UI에 표시하고, 저장 credential이 있는 profile 선택 시 token을 OS credential store에서 읽어 private temp token file에 쓴 뒤 sidecar를 `KUVIEWER_SOURCE=kubernetes`, `KUVIEWER_KUBE_API_SERVER`, `KUVIEWER_KUBE_TOKEN_FILE`로 재시작한다. 성공한 profile status는 `sidecar-kubernetes-active`가 되고 UI는 sidecar profile을 다시 읽어 live mode로 전환한다. `desktop_delete_kubernetes_profile_credential`는 native credential을 삭제하며, 활성 profile 삭제 시 Kubernetes sidecar를 중지하고 runtime token file을 정리한 뒤 기본 sidecar로 fallback한다. Runtime token file policy는 `0600-temp-dir-delete-on-sidecar-stop`이다. `KUVIEWER_DESKTOP_KUBE_TOKEN_FILE`과 `KUVIEWER_DESKTOP_KUBE_IMPORT_TOKEN_FILE=1`을 startup 전에 설정하면 Rust가 token file을 읽어 macOS Keychain 또는 Windows Credential Manager에 저장하지만 token content는 브라우저로 보내지 않는다. `scripts/smoke-desktop-keychain-runtime.mjs`는 stubbed Tauri bridge로 profile select/live token handoff/active credential delete fallback을 CI에서 자동 검증하며, 실제 OS credential store 항목은 만들지 않는다. 브라우저는 safe profile metadata만 보고 kubeconfig/body/token/private key를 저장하지 않는다. CI는 desktop packaging spec check, native credential runtime smoke, sidecar build dry-run check를 실행한다.
...
3. 업로드 모드 강화
   ...
   - 다음 확장 후보는 desktop package smoke matrix 확장이다.
