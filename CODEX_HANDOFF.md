...
3. 업로드 모드 강화
   ...
   - Pod logs 1차 범위, previous logs 조회, current-log follow, 로컬 로그 필터링, timestamp parsing/time range, sort toggle, raw line copy/download UX, 로그 표시 밀도 전환은 완료됐다.
   ...
   - 다음 확장 후보는 live log pause/resume UX 또는 resource detail layout polish다.
