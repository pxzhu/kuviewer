...
3. 업로드 모드 강화
   ...
   - Deploy upload timeout hardening은 완료됐다. 태그 배포의 SCP image upload는 attempt마다 300초 timeout을 적용하고 최대 3회 재시도하며, timeout은 `scp-upload-timeout`, 기타 실패는 `scp-upload-failed` safe marker로 표시한다. 실패/재시도 전후 partial remote image tar는 삭제하고, raw logs/secrets/env 파일은 출력하지 않는다.
