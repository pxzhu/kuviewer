...
3. 업로드 모드 강화
   ...
   - Desktop-only CM/SSH session manager metadata-only 1차 구현은 완료 대상이다.
   - 다음 확장 후보는 desktop SSH credential store + connection check, 그 다음은 CM session tunnel/runtime integration이다.
