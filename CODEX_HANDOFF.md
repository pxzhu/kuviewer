...
- Service type/ClusterIP/ExternalName/port/selector 검증과 summary/status는 `kubernetes_service.go`가 담당한다. type 기본값과 protocol 기본값을 정규화하고 canonical IP, DNS name, port/name/protocol, selector syntax/cardinality를 제한하며 invalid spec은 Pod 비교 전에 차단하고 이름 없는 kind-level diagnostic만 남긴다.
