...
3. 업로드 모드 강화
   ...
   - Deploy upload timeout hardening은 완료됐다. 태그 배포의 image upload는 attempt마다 300초 timeout을 적용하고 총 3회 시도하며, 전체 upload step은 18분으로 제한한다. 첫 시도는 SCP를 사용하고, SCP가 timeout/fail이면 이후 시도는 SSH command-channel stream fallback(`cat >` remote tar)을 사용한다. SCP timeout은 `scp-upload-timeout`, SCP 기타 실패는 `scp-upload-failed`, fallback timeout은 `ssh-stream-upload-timeout`, fallback 기타 실패는 `ssh-stream-upload-failed`, 성공 검증은 `upload-remote-image-verified` safe marker로 표시한다. 성공 업로드는 remote `gzip -t`로 확인하고, 실패/재시도 전후 partial remote image tar는 삭제하며, raw logs/secrets/env 파일은 출력하지 않는다.
