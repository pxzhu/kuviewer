...
- 설치형 클러스터 탐색 앱 packaging spike와 Tauri desktop shell scaffold는 `desktop/packaging-spec.json`, `desktop/README.md`, `desktop/package.json`, `desktop/src-tauri/*`, `scripts/check-desktop-packaging-spec.mjs`로 시작했다. 권장 방향은 Tauri-first, fallback은 Electron이며, 첫 대상은 macOS `.dmg`와 Windows `.exe`다. `desktop-readonly` capability는 shell/fs 권한 없이 `core:default`만 허용한다. CI는 desktop packaging spec check를 실행한다.
...
3. 업로드 모드 강화
   ...
   - 다음 확장 후보는 desktop icon/signing/build prerequisites 또는 saved view metadata polish다.
