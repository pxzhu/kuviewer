...
- 서버/클러스터 연결 시 리소스, 관계, Events, Logs를 읽기 전용으로 탐색할 수 있는 설치형 클러스터 탐색 앱을 장기 목표에 둔다. 후속 packaging 작업에서 macOS `.dmg`와 Windows `.exe` 배포 가능성을 검토한다.
...
3. 업로드 모드 강화
   ...
   - 다음 확장 후보는 selected keychain profile로 sidecar restart/live Kubernetes 연결 또는 desktop package smoke matrix 확장이다.
