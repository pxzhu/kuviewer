...
3. 업로드 모드 강화
   ...
   - Desktop deploy SSH preflight hardening은 완료됐다. 태그 배포는 Docker build 전에 SSH/host key/remote git-curl-gzip-Docker-compose/deploy path/.env/tmp write preflight를 실행하고, strict host key checking과 upload retry, always cleanup을 사용한다. Optional `SERVER_SSH_KNOWN_HOSTS` public host key pin을 지원하며, 없으면 6회 재시도, sequential key type scan, non-empty scan output 허용, IPv4 keyscan fallback을 사용한다. `scripts/prepare-deploy-known-hosts.mjs`는 host key pin 파일 생성/검증과 optional `gh secret set SERVER_SSH_KNOWN_HOSTS` 등록을 지원하고 key body는 콘솔에 출력하지 않는다. Manual `deploy-preflight` workflow는 release tag 없이 secrets/SSH/remote prerequisites만 검증하며 image build/upload/compose rollout/rollback은 실행하지 않는다.
