...
- 서버/클러스터 연결 시 리소스, 관계, Events, Logs를 읽기 전용으로 탐색할 수 있는 설치형 CM/SSH 세션 탐색 앱을 장기 목표에 둔다. macOS `.dmg`와 Windows `.exe` packaging 경로는 Tauri-first로 유지한다. 설치형 앱은 CM/SSH multiple sessions 관리가 목표이며, web app must not expose SSH. 기존 local sidecar/API 경로는 prototype-only로 둔다.
...
3. 업로드 모드 강화
   ...
   - Desktop package smoke matrix 확장은 완료 대상이다.
   - 다음 확장 후보는 desktop-only CM/SSH multiple sessions manager 구현이다.
