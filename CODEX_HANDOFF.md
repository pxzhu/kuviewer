...
- Service type/ClusterIP/ExternalName/port/selector 검증과 summary/status는 `kubernetes_service.go`가 담당한다. type/protocol/IP-family/traffic/session 기본값을 정규화하고 canonical single/dual-stack IP, family/policy 대응, DNS name, port/name/protocol, targetPort, nodePort, appProtocol, traffic policy, health-check port, load-balancer class/allocation, ClientIP timeout, selector syntax/cardinality를 제한한다. invalid spec은 Pod 비교 전에 차단하고 이름 없는 kind-level diagnostic만 남긴다. Upload mode는 `serviceSchema.ts`의 같은 fail-closed 경계로 safe summary만 만들고 invalid/ExternalName selector edge를 만들지 않는다.
