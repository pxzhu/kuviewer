...
Desktop CM saved session layout per-row conflict actions are available only inside the desktop layout conflict preview. Each row can resolve incoming, keep current, or rename incoming independently, so unresolved conflicts stay visible while resolved rows update only safe layout preference metadata.
