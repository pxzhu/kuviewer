...
The desktop UI can search the session list in memory by safe metadata only and shows a compact selected-session summary for credential availability, runtime status, health, and last connection check state. Search text and summary display state are UI-only; they are not written to localStorage, sessionStorage, JSON export, logs, or the repository.
