...
The desktop UI can search the session list in memory by safe metadata only and shows a compact selected-session summary for credential availability, runtime status, health, last connection check state, and advanced diagnostics. Desktop CM diagnostic filtering narrows the list by the displayed diagnostic stage and severity; search text, filter values, and summary display state are UI-only and are not written to localStorage, sessionStorage, JSON export, logs, or the repository.
