...
The smoke verifies CM/SSH session save/select, private-key import command flow, connection check command flow, credential deletion, and session deletion through a stubbed Tauri bridge. It confirms the web runtime does not expose SSH session UI, checks that no admin token or legacy API profile is stored, and verifies private key bodies are not captured by browser state.
