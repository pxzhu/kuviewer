...
Desktop CM saved layout preset folder reorder status history timestamp filter preset help tooltip polish is desktop-only UI state. The visible help button includes a compact hover/focus tooltip linked with `aria-describedby`, with safe shortcut and focus-action text. Help tooltip metadata stays in browser memory/rendered DOM and adds no storage key, order field, export/import field, Tauri payload field, credential data, runtime profile, diagnostic history, Events, or logs.
