...
The Resource Explorer detail panel shows a compact resource identity header, active section indicator, and open section count. Users can expand every detail section, collapse every detail section, or restore the default detail sections (`Metadata`, `Status`, `Safe Preview`, `Relations`, `Events`) without changing resource data. Detail section open state is browser-memory UI state only and is not written to localStorage, URLs, saved views, team saved views, JSON export/import, or backend APIs.
