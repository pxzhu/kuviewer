...
- Future packaging goal: an installable read-only desktop cluster explorer for macOS `.dmg` and Windows `.exe` distribution
...
Kuviewer also tracks an installable read-only desktop cluster explorer path for users who want a local app that can connect to a Kuviewer server and inspect the connected cluster. The current Tauri-first scaffold is documented in [desktop/README.md](desktop/README.md), with a machine-checked target spec in [desktop/packaging-spec.json](desktop/packaging-spec.json) and build prerequisites in [desktop/BUILD_PREREQUISITES.md](desktop/BUILD_PREREQUISITES.md).

The default packaging direction is Tauri first, with Electron as a fallback only if needed. Initial release targets are macOS `.dmg` and Windows `.exe`. The scaffold points at the existing Vite UI and keeps a `desktop-readonly` capability with no frontend shell or filesystem permissions. Desktop icons are generated from the transparent YAML Flow PNG asset, committed under `desktop/src-tauri/icons`, and checked by the packaging spec validator. A manual desktop package workflow can build unsigned `.dmg` / `.exe` artifacts, resolve package versions from a workflow input or `v*` tag ref, and include that version in uploaded artifact names. When explicitly enabled, it can also import signing certificates from CI secrets, run macOS notarization, and clean runner signing material after packaging. The first unsigned macOS `.dmg` and Windows `.exe` dry-runs succeeded on 2026-06-19. The desktop shell can store one remote server URL profile for connecting to an existing Kuviewer server, and the Go API server is now configured as a Rust-managed localhost sidecar for the installable app. The sidecar binds to `127.0.0.1:18086`, gets a per-launch admin token, and hands that token to the UI through a Tauri command so the browser keeps it in `sessionStorage` only. The desktop profile panel shows the local sidecar source and lets the user explicitly switch back from a remote server profile to the local sidecar profile without exposing the sidecar token as saved metadata. Default desktop sidecar source is `mock`; packaging smoke can opt into `KUVIEWER_DESKTOP_SIDECAR_SOURCE=kubernetes` once a safe cluster credential source is configured outside the browser. Direct desktop-to-cluster credential handling is designed in [desktop/KEYCHAIN_CREDENTIAL_DESIGN.md](desktop/KEYCHAIN_CREDENTIAL_DESIGN.md): secret material belongs in macOS Keychain or Windows Credential Manager, while the browser sees only safe profile metadata and the sidecar receives `KUVIEWER_KUBE_TOKEN_FILE` runtime temp files. The current prototype exposes safe metadata through `desktop_kubernetes_profiles` and `desktop_select_kubernetes_profile`, deletes native credentials through `desktop_delete_kubernetes_profile_credential`, imports a local token file into the OS store with `KUVIEWER_DESKTOP_KUBE_TOKEN_FILE` plus `KUVIEWER_DESKTOP_KUBE_IMPORT_TOKEN_FILE=1`, and restarts the sidecar with `KUVIEWER_SOURCE=kubernetes`, `KUVIEWER_KUBE_API_SERVER`, and `KUVIEWER_KUBE_TOKEN_FILE` when a stored profile is selected. `scripts/smoke-desktop-keychain-runtime.mjs` now runs a stubbed Tauri bridge smoke in CI to verify profile select, live-mode token handoff, and active credential delete fallback without touching a real OS credential store. Runtime token files follow `0600-temp-dir-delete-on-sidecar-stop`. The app must stay read-only without persisting kubeconfigs, admin tokens, cloud credentials, private keys, Secret values, Events, or logs.
