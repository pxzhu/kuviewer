...
The Resource Explorer detail panel shows a compact resource identity header, active section indicator, and open section count. Users can expand every detail section, collapse every detail section, or restore the default detail sections (`Metadata`, `Status`, `Safe Preview`, `Relations`, `Events`) without changing resource data. Safe Preview supports local key/value search with match highlighting and previous/next navigation for the selected resource. Detail section open state and Safe Preview search state are browser-memory UI state only and are not written to localStorage, URLs, saved views, team saved views, JSON export/import, or backend APIs.
