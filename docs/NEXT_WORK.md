...
1. Live Kubernetes connection verification
   ...
   - Service schema/summary/status는 live/upload 독립 module에서 type 기본값, canonical single/dual-stack ClusterIP, IP family/policy 대응, ExternalName, port/name/protocol, targetPort/nodePort/appProtocol, selector syntax/cardinality를 검증한다. invalid spec과 ExternalName selector는 Pod 비교 전에 차단하고 원격 원문 대신 `invalid`/safe count summary와 kind-level diagnostic만 남긴다. 다음 provider 감사는 traffic policy, session affinity, healthCheckNodePort와 loadBalancerClass 경계를 우선한다.
