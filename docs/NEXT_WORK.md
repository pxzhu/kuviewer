...
1. Live Kubernetes connection verification
   ...
   - Workload Pod template image/reference/status는 live/upload 독립 module에서 container/image/ServiceAccount/imagePullSecret/env/volume collection과 identity를 검증한다. raw env value/key는 보존하지 않고 malformed template은 warning/invalid summary와 phantom edge 억제로 fail-closed 처리한다. 다음 provider 감사는 Pod runtime container state/reason summary 경계를 우선한다.
