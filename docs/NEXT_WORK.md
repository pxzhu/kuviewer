...
1. Live Kubernetes connection verification
   ...
   - Pod runtime state/reason/restart/image는 live/upload 독립 module에서 phase, current/last state union, container identity, count와 image 문법을 검증한다. status message, imageID, containerID는 보존하지 않고 malformed runtime status는 warning/invalid summary와 kind-level diagnostic으로 fail-closed 처리한다. 다음 provider 감사는 Node capacity/allocatable/runtime info summary 경계를 우선한다.
