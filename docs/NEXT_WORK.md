...
1. Live Kubernetes connection verification
   ...
   - Node capacity/allocatable/condition/runtime summary는 live/upload 독립 module에서 quantity, count, version, runtime 문법과 collection 상한을 검증한다. 주소와 machine/system/boot identifier, condition message는 보존하지 않고 malformed status는 warning/invalid summary와 kind-level diagnostic으로 fail-closed 처리한다. 다음 provider 감사는 PV/PVC/StorageClass capacity, access mode, reclaim policy, provisioner 경계를 우선한다.
