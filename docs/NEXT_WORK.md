...
1. Live Kubernetes connection verification
   ...
   - Kubernetes graph builder는 독립 module/test로 분리돼 node/edge dedupe, dangling-edge 거부, safe Secret placeholder, metadata copy, layout lane을 검증한다. 다음 provider 축소는 snapshot resource assembly와 resource schema 분리를 우선한다.
