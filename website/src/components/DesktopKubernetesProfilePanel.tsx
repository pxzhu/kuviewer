import { CheckCircle2, KeyRound, ServerCog, ShieldCheck, Trash2, Unplug } from 'lucide-react';
...
export function DesktopKubernetesProfilePanel({
  message,
  profiles,
  onDeleteCredential,
  onSelectProfile,
}: DesktopKubernetesProfilePanelProps) {
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        ...
        {selectedProfile?.credentialAvailable ? (
          <span className="ku-chip max-w-full border-[rgba(52,199,89,0.22)] bg-[rgba(52,199,89,0.1)] text-[#248a3d]">
            <KeyRound size={13} aria-hidden="true" />
            native credential 있음
          </span>
        ) : null}
        {message ? (
      ...
      {profiles.length > 0 ? (
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          {profiles.map((profile) => (
            ...
            >
              <button className="min-w-0 text-left" type="button" onClick={() => onSelectProfile(profile.id)}>
              ...
              {profile.credentialAvailable ? (
                <button
                  ...
                  data-testid={`desktop-kubernetes-profile-delete-${profile.id}`}
                  type="button"
                  onClick={() => onDeleteCredential(profile.id)}
                >
